...
class offline_multiDataSet(Dataset):
  ...
  def __getitem__(self, item):
    ...
    # drr_filepath=os.path.join(self.dataroot, self.dataset_paths[item], 'drr.mhd')
    # first_angle=0
    # os.system(f'/home/zmx/RTK/rtksimulatedgeometry -f {str(first_angle)} -n 1 --arc {str(first_angle)} -o {geometry_filepath} && /home/zmx/RTK/rtkforwardprojections -g {geometry_filepath} -i {rtkct_filepath} -o {drr_filepath} --dimension=768 --fp=CudaRayCast')
    # drr_img=sitk.ReadImage(drr_filepath)
    # drr_array=sitk.GetArrayFromImage(drr_img)
